...
fileList = {'Slide1b.jpg','Slide2b.jpg','Slide3b.jpg','Slide4c.jpg','Slide5b.jpg','Slide6b.jpg'};
...
for fileIdx = 1:length(fileList)
    ...
    imhsv = rgb2hsv(im);
    imThsh1 = (imhsv(:,:,1) < 0.09) | (imhsv(:,:,1) > .97) & (imhsv(:,:,3) > 0.5);
    imThsh1 = imerode(imThsh1,strel('disk',2));
    ...
    % remove scar regions
    outlineMask = imThsh1;
    [sortedAreas,sortIdx] = sort([rp.Area]);
    scarRegions = sortIdx(find(sortedAreas > 30000));
    
    for scarIdx = 1:length(scarRegions)
        
        % remove scar regions from mask
        imScarMask = zeros(size(imThsh1));
        
        thisRP = rp(scarRegions(scarIdx));
        bb = thisRP.BoundingBox;
        imScarMask(ceil(bb(2))+[0:floor(bb(4))-1],ceil(bb(1))+[0:floor(bb(3))-1]) = thisRP.Image;
        
        outlineMask = outlineMask & ~imScarMask;        
    end
    imageData(fileIdx).outlineMask = outlineMask;
    
     % get regions
    rp = regionprops(outlineMask,'Area','BoundingBox','Image');
